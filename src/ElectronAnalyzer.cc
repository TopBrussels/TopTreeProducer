...
void ElectronAnalyzer::Process(const edm::Event& iEvent, TClonesArray* rootElectrons, EcalClusterLazyTools& lazyTools, const edm::EventSetup& iSetup )
{
  ...
  for (unsigned int j=0; j<nElectrons; j++)
	...
	// CiC working points
	localElectron.setIDCiCVeryLoose(int(patElectron->electronID("eidVeryLooseMC")));
	localElectron.setIDCiCLoose(int(patElectron->electronID("eidLooseMC")));
	localElectron.setIDCiCMedium(int(patElectron->electronID("eidMediumMC")));
	localElectron.setIDCiCTight(int(patElectron->electronID("eidTightMC")));
	localElectron.setIDCiCSuperTight(int(patElectron->electronID("eidSuperTightMC")));
	localElectron.setIDCiCHyperTight1(int(patElectron->electronID("eidHyperTight1MC")));
	
	// Original working points
	localElectron.setIDWP95(int(patElectron->electronID("simpleEleId95relIso")));
	localElectron.setIDWP90(int(patElectron->electronID("simpleEleId90relIso")));
	localElectron.setIDWP85(int(patElectron->electronID("simpleEleId85relIso")));
	localElectron.setIDWP80(int(patElectron->electronID("simpleEleId80relIso")));
	localElectron.setIDWP70(int(patElectron->electronID("simpleEleId70relIso")));
	localElectron.setIDWP60(int(patElectron->electronID("simpleEleId60relIso")));
