...
int main(int argc, char *argv[]){
  ...
  for (int i=1; i<eventTree->GetListOfBranches()->GetEntriesFast(); i++) {
    ...
    //cout << "  Loaded Branch " << className << " " << ObjName.substr(0,position) << endl;
    ...
    eventTree->SetBranchStatus((ObjName.substr(0,position)+'*').c_str(),1);
    eventTree->SetBranchAddress((ObjName.substr(0,position)+'*').c_str(),&objects);
